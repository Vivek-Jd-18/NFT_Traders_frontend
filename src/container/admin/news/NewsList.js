    ...
    Index.Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await Index.DataService
          .post(Index.Api.Admin.DeleteNews, urlencoded)
          .then((data) => {
            if (data.data.status === 1) {
              setCount((prevState) => prevState + 1);
              Index.toast.success("News deleted successfully", { theme: "dark" });
              getNewsList();
            }
          });
      }
      ...
      <Index.Container maxWidth="false">
        <Index.Grid container justify="flex-end">
          ...
          <Index.Grid>
            <Index.Button
              ...
              variant="contained"
              onClick={() => {
                navigate("/admin/createUpdateNews");
              }}
            >
              Add News
            </Index.Button>
        ...
        <Index.TableContainer component={Index.Paper}>
          ...
          >
            <Index.TableHead>
              <Index.TableRow>
                <StyledTableCell sx={{ width: "100px" }}>Name</StyledTableCell>
            ...
            <Index.TableBody>
              ...
              list.map((row, i) => (
                <StyledTableRow key={i}>
                  ...
                  <StyledTableCell>{Index.parse(row?.description.substring(0,250)+'...')}</StyledTableCell>
                  <StyledTableCell>{formatDate(row?.createdAt)}</StyledTableCell>
                  <StyledTableCell>
                    <Index.Button
                      onClick={() =>
                        navigate(`/admin/createUpdateNews/${row._id}`)
                      }
                      sx={{
                        marginRight: "10px",
                        padding: "0",
